...
const Welcome = () => {
  ...
  const handleReconnect = async () => {
    ...
    console.log("[Welcome] Manual reconnect: calling supabase.auth.getSession()");
    try {
      ...
      if (error) {
        toast({ title: "Erreur de reconnexion", description: error.message, variant: "destructive" });
      } else if (data.session) {
        toast({ title: "Session retrouvée", description: `Connecté en tant que ${data.session.user.email ?? data.session.user.id}` });
      } else {
        toast({
          title: "Aucune session active",
          description: "Aucune session Supabase n'existe dans ce navigateur. Connectez-vous via « Se connecter ».",
          variant: "destructive",
        });
      }
    } catch (err: any) {
  ...
  if (!loading && user) {
    ...
    return <Navigate to={target === "/welcome" ? "/" : target} replace />;
  }
    ...
    <div className="min-h-screen bg-gradient-hero flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl">
        <div className="text-center mb-10 animate-fade-in">
          <img src={abbiattiLogo} alt="Abbiatti" className="mx-auto mb-5 h-24 w-auto" />
          <h1 className="sr-only">Abbiatti</h1>
          {lastOrgName ? (
            <p className="text-lg font-medium text-primary">Bienvenue {lastOrgName} 👋</p>
          ) : (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-6 text-white shadow-xl hover-scale animate-fade-in">
            ...
            <div className="space-y-2">
              <Button
                ...
                onClick={() => navigate(buildRedirectQueryPath("/auth?tab=login", redirectTo), { state: location.state })}
              >
              ...
              {!lastOrgName && (
                <Button
                  ...
                  onClick={() => navigate(buildRedirectQueryPath("/auth?tab=signup", redirectTo), { state: location.state })}
                >
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-6 text-white shadow-xl hover-scale animate-fade-in">
            ...
            <Button
              ...
              onClick={() => navigate(buildRedirectQueryPath("/staff-login", redirectTo), { state: location.state })}
            >
